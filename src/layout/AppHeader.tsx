...
const AppHeader = (props: AppHeaderProps) => {
	...
	useEffect(() => {
		if (width < 900 && !confIsMobile) {
			appConfig.setConfig("isMobile", true);
		}
		if (width >= 900 && confIsMobile) {
			appConfig.setConfig("isMobile", false);
		}
	}, [width, confIsMobile]);
					...
					{!isMobile && (
						<Group gap={20} className={classes.headermenu}>
							{HEADERLINKS.map((link) => {
								const isActive = link.to !== "/" ? router.location.pathname.startsWith(link.to) : router.location.pathname === "/";
								return (
									<Button
										variant={isActive ? "filled" : !home ? "subtle" : "filled"}
										color={
											!home ? "var(--mantine-primary-filled)" : isActive ? "var(--mantine-primary-filled)" : "var(--mantine-color-body)"
										}
										key={link.to}
										to={link.to}
										component={Link}
